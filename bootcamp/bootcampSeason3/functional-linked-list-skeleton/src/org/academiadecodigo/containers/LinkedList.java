...
//map is to be returned , mapper is before, R is the type

        LinkedList<R> mapped = new LinkedList<R>();

        for (int i = 0; i < this.size(); i++) {
            mapped.add(mapper.apply(this.get(i)));
        }
        return mapped;
    ...
    public LinkedList<T> filter(Predicate<T> predicate) {
        ...
        LinkedList<T>  predicated = new LinkedList<>();

        for (int i = 0; i < this.size(); i++) {

            if(predicate.test(this.get(i))){
                predicated.add(this.get(i));
            }
    ...
    public T reduce(BinaryOperator<T> reducer, T accumulator) {
        ...
        for (int i = 0; i < this.size(); i++) {
           reduced = reducer.apply(reduced,(this.get(i)));
        }
        return reduced;
    }
